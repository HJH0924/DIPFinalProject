...
def add_new_employee(name):
    ...
    newEmp = o.Employee(o.get_new_id(), name, code)  # 创建教职工对象
    o.add(newEmp)  # 组织结构中添加新教职工
    io.save_employee_all()  # 保存最新的教职工信息
    return code  # 新教职工的特征码


# 删除某个教职工
def remove_employee(id):
    io.remove_pics(id)  # 删除该教职工所有图片
...
def get_employee_report():
    ...
    report += "教职工名单如下：\n"
    i = 0  # 换行计数器
    for emp in o.EMPLOYEES:  # 遍历所有教职工
...
def get_day_report(date):
    ...
    for emp in o.EMPLOYEES:  # 遍历所有教职工
        if emp.name in o.LOCK_RECORD.keys():  # 如果该教职工有打卡记录
            emp_lock_list = o.LOCK_RECORD.get(emp.name)  # 获取该教职工所有的打卡记录
            is_absent = True  # 缺勤状态
            ...
            if is_absent:  # 如果仍然是缺勤状态
                absent_list.append(emp.name)  # 加入缺勤名单
        else:  # 该教职工没有打卡记录
            absent_list.append(emp.name)  # 加入缺勤名单

    emp_count = len(o.EMPLOYEES)  # 教职工总人数
    print("--------" + date + "--------")
    print("应到人数：" + str(emp_count))
    print("缺勤人数：" + str(len(absent_list)))
    absent_name = ""  # 缺勤名单
    if len(absent_list) == 0:  # 如果没有缺勤的
        absent_name = "(空)"
    else:  # 有缺勤的
        for name in absent_list:  # 遍历缺勤列表
...
def get_month_report(month):
    ...
    lock_report = dict()  # 键为教职工名，值为教职工打卡情况列表

    for emp in o.EMPLOYEES:
        emp_lock_data = []  # 教职工打卡情况列表
        if emp.name in o.LOCK_RECORD.keys():  # 如果教职工有打卡记录
            emp_lock_list = o.LOCK_RECORD.get(emp.name)  # 从打卡记录中获取该教职工的记录
            ...
            while index_day <= month_last_day:
                ...
                if is_absent:  # 如果缺勤
                    emp_today_data = absent  # 直接赋予缺勤标志
                emp_lock_data.append(emp_today_data)  # 教职工打卡标记添加到打卡情况列表中
            ...
            while index_day <= month_last_day:  # 遍历整月
                emp_lock_data.append(absent)  # 每天都缺勤
                index_day = index_day + datetime.timedelta(days=1)  # 日期递增
        lock_report[emp.name] = emp_lock_data  # 将打卡情况列表保存到该教职工之下
    ...
    for emp in lock_report.keys():  # 遍历报表中的所有教职工
        report += "\"" + emp + "\""  # 第一列为教职工名
        data_list = lock_report.get(emp)  # 取出教职工的打卡情况列表
        for data in data_list:  # 取出每一天的打卡情况
            text = ""  # CSV中显示的内容
            if absent == data:  # 如果是缺勤
                text = "【缺勤】"
...
def load_emp_data():
    ...
    io.load_employee_info()  # 载入教职工信息
    io.load_employee_pic()  # 载入教职工照片
