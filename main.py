...
---------------------------------------------------"""
    while True:
        ...
        if option == "1":  # 如果选择“录入新教职工”
            name = str(input("请输入新教职工姓名(输入0取消操作)：")).strip()
            if name != "0":  # 只要输入的不是0
                code = hr.add_new_employee(name)  # 人事服务添加新教职工，并获得该教职工的特征码
                print("请面对摄像头，敲击三次回车键完成拍照！")
                camera.register(code)  # 打开摄像头为员工照相
                print("录入成功！")
                # return  # 退出教职工管理功能菜单
        elif option == "2":  # 如果选择“删除教职工”
            # show_employee_all()  # 展示教职工列表
            print(hr.get_employee_report())  # 打印教职工信息报表
            id = int(input("请输入要删除的教职工编号(输入0取消操作)："))
            if id > 0:  # 只要输入的不是0
                if hr.check_id(id):  # 若此编号有对应教职工
                    ...
                    if str(verification) == str(inputVer).strip():  # 如果验证码正确
                        hr.remove_employee(id)  # 人事服务删除该教职工
                        print(str(id) + "号教职工已删除！")
                    else:  # 无效编号
                        print("验证码有误，操作取消")
                else:
                    print("无此教职工，操作取消")
...
|                 查看记录功能菜单                |
+-------------------------------------------------+
  ①查看教职工列表  ②查看打卡记录  ③返回上级菜单
---------------------------------------------------"""
    while True:
        print(menu)  # 打印菜单
        option = input("请输入菜单序号：")
        if option == "1":  # 如果选择“查看教职工列表”
            print(hr.get_employee_report())  # 打印教职工信息报表
...
---------------------------------------------------"""
    while True:
        ...
        if option == "1":  # 如果选择“日报”
            while True:
                date = input("输入查询日期，格式为(2021-12-12),输入0则查询今天：")
                if date == "0":  # 如果只输入0
            ...
            while True:
                date = input("输入查询月份，格式为(2021-12),输入0则查询上个月：")
                if date == "0":  # 如果只输入0
...
def face_clock():
    print("请正面对准摄像头进行打卡")
    name = camera.clock_in()  # 开启摄像头，返回打卡教职工名称
    if name is not None:  # 如果教职工名称有效
    ...
    while not finish:
        ...
        if option == "1":  # 如果选择“打卡”
            ...
            if ADMIN_LOGIN:  # 如果管理员已登录
                ...
                login()  # 先让管理员登录
        elif option == "3":  # 如果选择“教职工管理”
            if ADMIN_LOGIN:
                employee_management()  # 进入教职工管理方法
            else:
                login()
