...
DATA_FILE = PATH + "employee_data.txt"  # 教职工信息文件
...
def checking_data_files():
    ...
    if not os.path.exists(DATA_FILE):
        ...
        print("教职工信息文件丢失，已重新创建：" + DATA_FILE)
    ...
    if not os.path.exists(WORK_TIME):
        ...
        file.write("08:00:00/16:00:00")  # 将默认时间写入到文件中
        file.close()  # 关闭文件
...
def load_employee_info():
    ...
    for line in file.readlines():  # 遍历文件中的行内容
        ...
        o.add(o.Employee(id, name, code))  # 组织结构中添加教职工信息
        if int(id) > max_id:  # 如果发现某教职工的id更大
...
def load_employee_pic():
    ...
    if len(pics) != 0:  # 如果照片文件不是空的
        ...
        print("Error >> 教职工照片文件丢失，请重新启动程序并录入教职工信息！")


# 将教职工信息持久化
def save_employee_all():
    file = open(DATA_FILE, "w", encoding="utf-8")  # 打开教职工信息文件，只写，覆盖
    info = ""  # 待写入的字符串
    for emp in o.EMPLOYEES:  # 遍历所有员工信息
        # 拼接教职工信息
        info += str(emp.id) + "," + str(emp.name) + "," + str(emp.code) + "\n"
    file.write(info)  # 将这些教职工信息写入到文件中
    file.close()  # 关闭文件
...
def remove_pics(id):
    ...
    code = str(hr.get_code_with_id(id))  # 获取该教职工的特征码
